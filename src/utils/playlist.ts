...
export async function addSongToPlaylist(playlistName: string, songPath: string) {
  ...
  export async function removeSongFromPlaylistByIndex(playlistName: string, songIndex: number) {
    const targetDirectory = `${PLAYER_DIRECTORY}\\${PLAYER_PLAYLIST_DIRECTORY}`;
    const filePath = join(targetDirectory, `${playlistName}.mdl`);
    ...
    if (songIndex < 1 || songIndex > songs.length) {
      throw new Error(`Invalid song index: ${songIndex}. Playlist contains ${songs.length} songs.`);
    }
  
    // Remove the song by index (adjust for 0-based array indexing)
    const updatedSongs = songs.filter((_, index) => index !== songIndex - 1).join('\n');
  ...
  export async function getAllPlaylists() {
    ...
    const playlists = files
      ...
      .map((file, index) => {
        ...
        return getSongsFromPlaylist(playlistName)
          .then(songs => ({
            index: index,
